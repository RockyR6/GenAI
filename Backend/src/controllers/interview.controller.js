...
async function generateRusumePdfController(req, res) {
  try {
    ...
    res.status(500).json({
      message: "Error generating interview reports.",
      error: error.message,
    });
  }
}
...
/**
 * @description Controller to delete the interview report.
 */
async function deleteInterviewReportController(req, res) {
  try {
    const { interviewId  } = req.params;
    const deletedReport = await InterviewReportModel.findByIdAndDelete({
      _id: interviewId,
      user: req.user.id,
    });
    ...
    res.status(500).json({
      message: "Error deleting interview reports.",
      error: error.message,
    });
  }
}
